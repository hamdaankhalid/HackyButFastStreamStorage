...
namespace StreamDB
{
    ...
    public interface IStreamDb : IDisposable
    {
        ...
        /// temporary memory — no <c>byte[]</c> is allocated per entry. Late arrivals are merged
        /// in primary index order. The handler must process or copy the payload inline; it is

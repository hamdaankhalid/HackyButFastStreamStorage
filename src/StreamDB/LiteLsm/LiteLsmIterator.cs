...
public sealed class LiteLsmIterator<TKey, TValue> : IDisposable where TKey : unmanaged, IComparable<TKey> where TValue : unmanaged
{
    ...
    private readonly KeyValuePair<TKey, TValue>[] _bufferedBatch;
    ...
    public LiteLsmIterator(LiteLsm<TKey, TValue> storage, TKey fromKey, TKey endKey, int batchSize = DEFAULT_BATCH_SIZE)
    {
        _storage = storage;
        _endKey = endKey;
        _readFrom = fromKey;
        _batchSize = batchSize;
        _bufferedBatch = ArrayPool<KeyValuePair<TKey, TValue>>.Shared.Rent(batchSize);
    }
    ...
    /// </summary>
    public IEnumerable<KeyValuePair<TKey, TValue>> ReadAll()
    {
        while (true)
        {
            ...
            yield return _bufferedBatch[_bufferedBatchOffset++];
        }
    }
    ...
    private void ReadNextBatch()
    {
        ...
        var fullBuffer = new Span<KeyValuePair<TKey, TValue>>(_bufferedBatch, 0, _batchSize);
        ...
        {
            _readFrom = _bufferedBatch[_bufferedBatchCount - 1].Key;
            _isSubsequentBatch = true;
        }
    }

    public void Dispose() => ArrayPool<KeyValuePair<TKey, TValue>>.Shared.Return(_bufferedBatch);
}